...
function renderShell() {
  ...
  shell.innerHTML =
    ...
    '  <div class="topbar__inner topbar__inner--premium">' +
    '    <button class="mobile-menu-btn icon-btn" id="mobile-menu-btn" aria-label="Open navigation">' + icon('menu') + '</button>' +
    '    <a href="#/" class="brand brand--premium">' +
    '      <span class="brand__mark brand__mark--logo">CDP</span>' +
    '      <span class="brand__copy"><strong>Construction Document</strong><small>Portal</small></span>' +
    '    </a>' +
    '    <form id="global-search" class="topbar__search topbar__search--premium">' +
    '      ' + icon('search') + '<input type="text" id="global-search-input" name="q" placeholder="Search projects, contractors, documents..." autocomplete="off">' +
    ...
    '      <a href="#/" class="side-link" data-nav="dashboard">' + icon('grid') + '<span>Dashboard</span></a>' +
    '      <a href="#/search" class="side-link" data-nav="search">' + icon('search') + '<span>Search</span></a>' +
    '      <a href="#/search/projects" class="side-link" data-nav="projects">' + icon('building') + '<span>Projects</span></a>' +
    '      <a href="#/contractors" class="side-link" data-nav="contractors">' + icon('users') + '<span>Contractors</span></a>' +
    '      <a href="#/search/documents" class="side-link" data-nav="documents">' + icon('doc') + '<span>Documents</span></a>' +
    '      <a href="#/search/photos" class="side-link" data-nav="photos">' + icon('camera') + '<span>Photos</span></a>' +
    '      <a href="#/search/categories" class="side-link" data-nav="categories">' + icon('grid') + '<span>Project Category</span></a>' +
    adminLinks +
    '    </div>' +
    '    <div class="system-status">' +
    '      <strong>System Status</strong>' +
    '      <span class="status-online"><i></i> Online</span>' +
    '      <small>Connected to live data</small>' +
    '    </div>' +
    '  </aside>' +
  ...
  var newBtn = document.getElementById('top-new-btn');
  if (newBtn) newBtn.addEventListener('click', function () {
    if (isStaff) openNewSystemModal();
  ...
  function closeSide() { document.body.classList.remove('sidebar-open'); }
  if (mobileBtn) mobileBtn.addEventListener('click', function () { document.body.classList.toggle('sidebar-open'); });
  if (backdrop) backdrop.addEventListener('click', closeSide);
  document.querySelectorAll('.side-link').forEach(function (a) { a.addEventListener('click', closeSide); });
  updateActiveSideNav();
}
...
var ROUTES = [
  ...
  { re: /^#\/contractors$/, fn: viewContractors },
...
function mount(html, mode) {
  var view = document.getElementById('view');
  view.classList.remove('dashboard-view', 'wide-view');
  if (mode) view.classList.add(mode + '-view');
  view.innerHTML = html;
  updateActiveSideNav();
  ...
  Promise.all([Api.get('dashboardStats', {}), ensureSearchIndexLoaded()])
    .then(function (res) {
      ...
      var html =
        ...
        '      <button class="dash-btn dash-btn--green" id="dash-upload-btn">' + icon('upload') + ' Upload Document</button>' +
        '      <button class="dash-btn dash-btn--blue" id="dash-add-btn">' + icon('plus') + ' Add Project</button>' +
        '      <button class="dash-btn dash-btn--light" id="dash-search-btn">' + icon('search') + ' Quick Search</button>' +
        '    </div>' +
        '  </section>' +
        ...
        '</div></div>' +
        '    <div class="dashboard-panel contractor-mini-panel"><div class="panel-head"><div><h2>Contractor Network</h2><p>Live contractor coverage</p></div><button class="mini-action" id="open-contractors-btn">' + icon('users') + ' Contractors <span>›</span></button></div><div class="contractor-mini-list">' +
          contractors.slice(0, 5).map(function (c) { return '<a href="#/contractor/' + encodeURIComponent(c.contractorId) + '" class="contractor-mini"><span class="contractor-avatar">' + escapeHtml((c.contractorName || 'C').charAt(0).toUpperCase()) + '</span><span><strong>' + escapeHtml(c.contractorName) + '</strong><small>' + escapeHtml(c.projectCount || 0) + ' projects · ' + escapeHtml(c.fileCount || 0) + ' files</small></span><span class="contractor-arrow">›</span></a>'; }).join('') +
          (contractors.length > 5 ? '<button class="more-contractors" id="more-contractors-btn">+' + (contractors.length - 5) + ' more contractors</button>' : '') +
        '</div></div>' +
        '  </section>' +
      ...
      if (uploadBtn) uploadBtn.addEventListener('click', function () { location.hash = '#/search'; });
      if (addBtn) {
        if (user.role === 'public' || user.role === 'contractor') {
          addBtn.innerHTML = icon('search') + ' Search Projects';
          addBtn.addEventListener('click', function () { location.hash = '#/search'; });
        } else {
          addBtn.addEventListener('click', function () { openNewSystemModal(); });
        }
      }
      document.getElementById('dash-search-btn').addEventListener('click', function () { document.getElementById('global-search-input').focus(); });
      document.getElementById('view-all-search').addEventListener('click', function () { location.hash = '#/search'; });
      document.getElementById('open-contractors-btn').addEventListener('click', function () { location.hash = '#/contractors'; });
...
function dashboardKpi(iconName, value, label, tone, meta) {
  return '<div class="kpi-card kpi-card--' + tone + '"><span class="kpi-icon">' + icon(iconName) + '</span><div class="kpi-copy"><small>' + escapeHtml(label) + '</small><strong>' + escapeHtml(value == null ? 0 : value) + '</strong><span class="kpi-meta">' + escapeHtml(meta) + '</span></div><span class="kpi-spark"><i></i><i></i><i></i><i></i><i></i></span></div>';
}
function quickAccessCard(iconName, tone, title, subtitle, href) {
...
function statusClass(status) {
  ...
  if (s.indexOf('complete') !== -1) return 'status-pill--green';
  if (s.indexOf('active') !== -1 || s.indexOf('ongoing') !== -1 || s.indexOf('progress') !== -1) return 'status-pill--blue';
  if (s.indexOf('pending') !== -1 || s.indexOf('hold') !== -1) return 'status-pill--amber';
  if (s.indexOf('cancel') !== -1 || s.indexOf('block') !== -1) return 'status-pill--red';
  return 'status-pill--gray';
}

function viewContractors() {
  mount(loadingHtml('Contractors লোড হচ্ছে...'), 'wide');
  ensureSearchIndexLoaded().then(function () {
    var contractors = State.allContractors || [];
    mount('<div class="page-heading"><div><span class="dashboard-kicker">PROJECT NETWORK</span><h1>Contractors</h1><p>All contractor records available from the live project index.</p></div><a class="dash-btn dash-btn--blue" href="#/search">' + icon('search') + ' Search</a></div>' +
      '<div class="contractors-page-grid">' + contractors.map(plateCard).join('') + '</div>', 'wide');
  }).catch(function (err) { mount(errorHtml(err.message), 'wide'); });
}
