...
function viewHome() {
  mou      // Greeting based on time of day
  ...
  var html =
    ...
    '    <div><span class="dashboard-kicker">CONSTRUCTION INTELLIGENCE PLATFORM</span><h1>' + greeting + ', ' + escapeHtml(user.name || 'Guest') + '! <span>👋</span></h1><p>Here\'s what\'s happening with your construction projects today.</p></div>' +
    ...
    // ─── Contractor Network — PROMINENT, moved up ───
    ...
    contractors.slice(0, 8).map(function (c) {
      return '<a href="#/contractor/' + encodeURIComponent(c.contractorId) + '" class="contractor-net-card">' +
        ...
        '<div class="contractor-net-info">' +
        '  <strong>' + escapeHtml(c.contractorName) + '</strong>' +
        '  <small class="mono">' + escapeHtml(c.contractorId) + '</small>' +
        '</div>' +
        '<div class="contractor-net-stats">' +
        '  <span>' + icon('building') + ' ' + (c.projectCount || 0) + '</span>' +
        '  <span>' + icon('doc') + ' ' + (c.fileCount || 0) + '</span>' +
        '</div>' +
  ...
  // "New Project" button — only for admin/moderator
  var addBtn = document.getElementById('dash-add-btn');
  if (addBtn) addBtn.addEventListener('click', function () { openNewSystemModal(); });
  document.getElementById('view-all-search').addEventListener('click', function () { location.hash = '#/search'; }); ('dash-upload-btn');
  var addBtn = document.getElementById('dash-add-btn');
